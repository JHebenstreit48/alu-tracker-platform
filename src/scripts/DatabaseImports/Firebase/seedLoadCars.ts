...
// ts-node/tsconfig-paths are registered in main entry

export async function loadCarsFromFile(file: string): Promise<SeedCar[]> {
  if (isJson(file)) {
    const raw = JSON.parse(fs.readFileSync(file, "utf8"));
    return asArray(raw).map((o) => (Array.isArray(o) ? (o[0] as SeedCar) : o));
  }
  ...
  if (isTsCollector(file)) {
    ...
    return asArray(data).map((o) => (Array.isArray(o) ? (o[0] as SeedCar) : o));
  }
