...
export default function BrandQuickList({ manufacturers }: BrandQuickListProps) {
  ...
  const groupedByCountry = manufacturers.reduce((acc: Record<string, { _id: string; brand: string; slug: string; }[]>, manufacturer) => {
  ...
  return (
    <div className="brand-quick-list">
      {sortedCountries.map((country) => {
        ...
        const groupedByLetter = countryManufacturers.reduce((acc: Record<string, { _id: string; brand: string; slug: string; }[]>, manufacturer) => {
